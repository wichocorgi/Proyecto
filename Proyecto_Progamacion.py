...
def datacollect3(busqueda, paginas):
    ...
    # Creamos un dataframe con toda la informacion que hemos recopilado
    df = pd.DataFrame(data)
    df.to_csv("datasets/productosCCMusic3.csv")
...
def promedio():
    ...
    # Hacemos un diccionario para poder nombrar los archivos
    diccionario = {
        ...
        try:
            ...
            if 'precio' in df.columns:
                ...
                if not df.empty:
                    ...
                    averages.append({'File': genre, 'Average Price': preciopromedio})
...
def mysqlconnect():
    ...
    # Eliminar la columna 0 de los datasets
    data1 = data1.drop(columns=['Unnamed: 0'])
    data2 = data2.drop(columns=['Unnamed: 0'])
    data3 = data3.drop(columns=['Unnamed: 0'])

    # limpiar la columnat]'
    data1['titulo'] = data1['titulo'].str.replace(r'\[.*\]', '', regex=True).str.strip()
    data2['titulo'] = data2['titulo'].str.replace(r'\[.*\]', '', regex=True).str.strip()
    data3['titulo'] = data3['titulo'].str.replace(r'\[.*\]', '', regex=True).str.strip()

    # Limpiar la columna 'precio'
    data1['precio'] = data1['precio'].str.replace(r'Price: \$|,', '', regex=True).astype(float)
    data2['precio'] = data2['precio'].str.replace(r'Price: \$|,', '', regex=True).astype(float)
    data3['precio'] = data3['precio'].str.replace(r'Price: \$|,', '', regex=True).astype(float)
...
def update_graph(chart_type):
    ...
    if chart_type == "bar":
        ...
        avggenre = data.groupby('File')['Average Price'].mean().reset_index()
        fig = px.bar(avggenre, x='File', y='Average Price', title="Promedio de Precios por Género Musical",
        ...
        for genre in genres:
            prom_gen = data[data['File'] == genre]['Average Price'].mean()
            prom_mercado = get_market_average(genre)  # Obtener el promedio del mercado
            comparison_data.append({
                'File': genre,
                'Average Price': prom_gen,
                'Market Average': prom_mercado
            })
...
if __name__ == "__main__":
    #Verificamos que existan los datasets
    path = 'datasets/productosCCMusic.csv'
    path2 = 'datasets/productosCCMusic2.csv'
    path3 = 'datasets/productosCCMusic3.csv'
    checkfile = os.path.isfile(path)
    checkfile2 = os.path.isfile(path2)
    checkfile3 = os.path.isfile(path3)
